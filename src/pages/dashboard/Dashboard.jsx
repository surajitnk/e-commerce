...
function Dashboard() {
    ...
    const {Allproduct,mode} = context
    const navigate = useNavigate()
    const deleteItem = async (e) => {
        try {
            await deleteDoc(doc(fireDb,'product',e.id))
        } catch (error) {
            console.log(error)
        }
    }
                                ...
                                <div className="relative overflow-x-auto ">
                                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400  ">
                                        ...
                                        {Allproduct.map((e ,index)=> (
                                            <tr className="bg-gray-50 border-b dark:border-gray-700" key={e.id} style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                                <td className="px-6 py-4 text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
                                                   {index+1}
                                                </td>
                                                <th scope="row" className="px-6 py-4 font-medium text-black whitespace-nowrap">
                                                    <img className='w-16' src= {e.imageUrl} alt="img" />
                                                </th>
                                                <td className="px-6 py-4 text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
                                                    {e.title}
                                                </td>
                                                ...
                                                <td className="px-6 py-4 text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
                                                {e.time && e.time.seconds && new Date(e.time.seconds * 1000).toLocaleDateString()}
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className=" flex gap-2">
                                                        <div className=" flex gap-2 cursor-pointer text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
                                                            <div  >
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} onClick={deleteItem} stroke="currentColor" className="w-6 h-6">
